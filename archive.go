...
// Archiver is used to create tar.gz archives from objects in memory
...
func (a *Archiver) AddCSV(filename string, lines [][]string) error {
	...
	if a.isDestroyed() {
		return errors.New("Archiver has been destroyed")
	}
...
func (a *Archiver) CreateArchive(filepath string) error {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	...
	filepath = strings.Split(filepath, ".")[0] + ".tar.gz"
